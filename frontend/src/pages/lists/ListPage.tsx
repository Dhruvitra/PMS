...
// Exported so other surfaces (e.g. global search) show the exact same status
// colors instead of a separately hand-maintained copy that drifts out of sync.
...
export function ListPage() {
  ...
  useEffect(() => {
    if (!socket) return;
    let t: any;
    const refresh = () => { clearTimeout(t); t = setTimeout(() => loadData(true), 500); };
  ...
  const handleRenameTask = async (taskId: string) => {
    ...
    } catch { toast.error('Failed to rename task'); loadData(true); }
  };
  ...
  const handleDeleteSingleTask = async (taskId: string) => {
    ...
    } catch { toast.error('Failed to delete task'); loadData(true); }
  };
  ...
  const handleToggleFavorite = async (task: Task) => {
    ...
    } catch { toast.error('Failed to update favorite'); loadData(true); }
  };
        ...
        {moveTaskId && (
          ...
          >
            <motion.div
              ...
              className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-[420px] max-h-[500px] flex flex-col"
              onClick={e => e.stopPropagation()}
            >
